<?php
    ...
    /* Public Variables */
    	var $smtp_port = 25;
   		var $time_out = 30; //is used in fsockopen() 
    	var $host_name = SMTP_SERVER;
		var $log_file;
		var $relay_host = SMTP_SERVER;
		var $debug = FALSE;//FALSE;
		var $auth = SMTP_AUTH;//FALSE;
		var $user = SMTP_USER;
		var $password = SMTP_PASSWORD;
		var $sender = SMTP_USER;
		...
		function Smtp($from = "")
		{
			$this->sender = $from?$from:$this->sender;
		}
		/* Main Function */
		function sendmail($to,$subject = "", $body = "", $cc = "", $bcc = "", $additional_headers = "")
		{
			$sent = TRUE;
			$mail_from = $this->get_address($this->strip_comment($this->sender));
			$body = ereg_replace("(^|(\r\n))(\.)", "\1.\3", $body);
		...
		{
			//die("helo :<br>$helo <br> from :<br>$from <br>to :<br>$to <br> header :<br>$header <br> body :<br>$body");

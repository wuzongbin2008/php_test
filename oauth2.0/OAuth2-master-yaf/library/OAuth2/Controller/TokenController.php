...
class TokenController implements TokenControllerInterface
{
    ...
    public function __construct(AccessTokenInterface $accessToken, ClientInterface $clientStorage,
                                array $grantTypes = array(), ClientAssertionTypeInterface $clientAssertionType = null,
    ...
    {
        $header_id = $request->headers('PHP_AUTH_USER');
        $header_secret = $request->headers('PHP_AUTH_PW');
        $client_id = isset($header_id) ? $header_id : $request->request('client_id');
        $client_secret = isset($header_secret) ? $header_secret : $request->request('client_secret');
...
//                'user_id' => $request->request('user_id'),
                'access_token' => $client_id . '|' . $token['access_token']
            ];
            $response->addParameters($access_token);
...
//            return null;
            $grantTypeIdentifier = 'authorization_code';
        }
